...
<?php else: ?>
    ...
    <div class="task-list">
        ...
        <?php
            $cardClass = 'task-card priority-' . $task['priority'] . ' status-' . $task['status'];
            if (isOverdue($task['deadline'], $task['status'])) $cardClass .= ' row-overdue';
        ?>
        <div class="<?= $cardClass ?>">
            <div class="task-card-top">
                <span class="task-title"><?= htmlspecialchars($task['title']) ?></span>
                <span class="badge <?= statusBadge($task['status']) ?>"><?= ucfirst($task['status']) ?></span>
            </div>
            <?php if (!empty($task['description'])): ?>
                <p class="task-desc"><?= htmlspecialchars(mb_strimwidth($task['description'], 0, 80, '...')) ?></p>
            <?php endif; ?>
            <div class="task-meta">
                <span class="badge <?= priorityBadge($task['priority']) ?>"><?= ucfirst($task['priority']) ?></span>
                ...
                <?php if (!empty($task['deadline'])): ?>
                    <span class="<?= isOverdue($task['deadline'], $task['status']) ? 'text-danger' : 'text-muted' ?>">
                        <?= formatDate($task['deadline']) ?><?= isOverdue($task['deadline'], $task['status']) ? ' ⚠️' : '' ?>
                    </span>
                <?php endif; ?>
            </div>
        ...
        <div class="card">
            <div class="table-wrapper">
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($tasks as $task): ?>
                        <tr class="<?= ($task['status'] === 'gedaan') ? 'row-done' : '' ?> <?= isOverdue($task['deadline'], $task['status']) ? 'row-overdue' : '' ?>">
                            <td>
